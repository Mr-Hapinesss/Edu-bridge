...
export default function EditCourse() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true)
      await updateCourse(id, { ...form, price: Number(form.price) || 0 })
    ...
    <div className="min-h-screen bg-amber-50 dark:bg-slate-900">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500 mb-6">
          <Link to="/instructor/dashboard" className="hover:text-teal-600 dark:hover:text-teal-400 transition-colors">My Courses</Link>
          <span>/</span>
          <span className="text-slate-600 dark:text-slate-300">Edit</span>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-amber-100 dark:border-slate-700 p-6 sm:p-8">
          ...
          {success && (
            <div className="mb-6 px-4 py-3 rounded-xl bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-700 text-teal-700 dark:text-teal-300 text-sm">
              {success}
            </div>
          )}
          {error && (
          ...
          <div className="space-y-5">
            <div>
              ...
              <input name="title" type="text" value={form.title} onChange={handleChange} className={inputClass} />
            </div>
            <div>
